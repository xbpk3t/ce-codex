...
1. **Run `/prompts:ce-review`** on the cumulative diff (baseline to final). Load the `ce:review` skill with `mode:autofix` on the optimization branch.
2. **Run `/prompts:ce-compound`** to document the winning strategy as an institutional learning.
